...
wget https://github.com/hugoguru/dist-hugo/releases/download/v${HUGO_VERSION}/hugo-standard-${HUGO_VERSION}-linux-${HUGOGURU_ARCH}.tar.gz
...
grep hugo-standard-${HUGO_VERSION}-linux-${HUGOGURU_ARCH}.tar.gz hugo-${HUGO_VERSION}-checksums-sha256.txt | sha256sum -c
...
tar -zxf hugo-standard-${HUGO_VERSION}-linux-${HUGOGURU_ARCH}.tar.gz -C /files/usr/lib/hugo
